...
> **分类**: 商品-能源 | **数据源**: FRED | **频率**: 日
> **FRED/API**: `DCOILWTICO` | **导入阈值**: 日变化 >1σ
> **数据库**: 350 条记录
...
油价↑ → 通胀预期↑ → 实际利率↓ → 金价↑ (间接正相关)

## 抓取代码

```python
df = web.DataReader('DCOILWTICO', 'fred', start=datetime.date(2019,12,1), end=datetime.date(2026,7,1))
...
FROM world_event
WHERE event_type = 'WTI_CRUDE_OIL_SPOT'
...
FROM world_event we
WHERE we.event_type = 'WTI_CRUDE_OIL_SPOT'
  AND we.severity >= 3
