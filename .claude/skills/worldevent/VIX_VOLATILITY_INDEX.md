...
> **分类**: 风险情绪-核心 | **数据源**: FRED | **频率**: 日
> **FRED/API**: `VIXCLS` | **导入阈值**: 日变化 >1σ
> **数据库**: 362 条记录

## 概述

CBOE波动率指数(恐慌指数)，衡量市场恐慌程度。

**可预测性**: 0.2 — 📊 市场驱动 — 连续定价，可被概率/技术/趋势预测

## 对金价的影响

VIX↑ → 恐慌情绪↑ → 避险资金流入黄金 → 金价↑ (正相关)

## 抓取代码

```python
df = web.DataReader('VIXCLS', 'fred', start=datetime.date(2019,12,1), end=datetime.date(2026,7,1))
...
FROM world_event
WHERE event_type = 'VIX_VOLATILITY_INDEX'
...
FROM world_event we
WHERE we.event_type = 'VIX_VOLATILITY_INDEX'
  AND we.severity >= 3
