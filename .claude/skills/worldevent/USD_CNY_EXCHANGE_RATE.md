...
> **FRED/API**: `DEXCHUS` | **导入阈值**: 日变化 >1σ
> **数据库**: 448 条记录

## 概述

美元兑人民币汇率，影响中国(全球最大黄金消费国)的购买力。

**可预测性**: 0.2 — 📊 市场驱动 — 连续定价，可被概率/技术/趋势预测

## 对金价的影响

USD/CNY↑(人民币贬值) → 中国进口黄金成本↑ → 需求可能↓ → 金价承压

## 抓取代码

```python
df = web.DataReader('DEXCHUS', 'fred', start=datetime.date(2019,12,1), end=datetime.date(2026,7,1))
...
FROM world_event
WHERE event_type = 'USD_CNY_EXCHANGE_RATE'
...
FROM world_event we
WHERE we.event_type = 'USD_CNY_EXCHANGE_RATE'
  AND we.severity >= 3
