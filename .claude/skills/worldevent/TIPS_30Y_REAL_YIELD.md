...
```python
df = web.DataReader('DFII30', 'fred', start=datetime.date(2019,12,1), end=datetime.date(2026,7,1))
...
FROM world_event
WHERE event_type = 'TIPS_30Y_REAL_YIELD'
...
FROM world_event we
WHERE we.event_type = 'TIPS_30Y_REAL_YIELD'
  AND we.severity >= 3
