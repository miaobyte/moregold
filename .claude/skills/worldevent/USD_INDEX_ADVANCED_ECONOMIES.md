# USD_INDEX_ADVANCED_ECONOMIES

> **分类**: 美元 | **数据源**: FRED | **频率**: 日
> **FRED/API**: `DTWEXAFEGS` | **导入阈值**: 日变化 >1σ
> **数据库**: 548 条记录
...
```python
df = web.DataReader('DTWEXAFEGS', 'fred', start=datetime.date(2019,12,1), end=datetime.date(2026,7,1))
...
FROM world_event
WHERE event_type = 'USD_INDEX_ADVANCED_ECONOMIES'
...
FROM world_event we
WHERE we.event_type = 'USD_INDEX_ADVANCED_ECONOMIES'
  AND we.severity >= 3
